...
class MinistryAdmin(admin.ModelAdmin):
    list_display = ['title', 'activity_date', 'location',  'is_active', 'display_order','display_image'] 
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('title', 'description', 'activity_date', 'location')
        }),
        ('顯示控制', {
...
class PrayerAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('代禱信息', {
            'fields': ('title', 'content', 'is_urgent', 'is_active')
        }),
        ('時間設置', {
            'fields': ('display_date',),
            'description': '設定顯示日期'
